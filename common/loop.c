...
struct loop *loop_create(void) {
	...
	if (!loop) {
		wmiiv_log(SWAY_ERROR, "Unable to allocate memory for loop");
		return NULL;
	}
	loop->fd_capacity = 10;
...
void loop_add_fd(struct loop *loop, int fd, short mask,
	...
	if (!event) {
		wmiiv_log(SWAY_ERROR, "Unable to allocate memory for event");
		return;
	}
	...
	if (loop->fd_length == loop->fd_capacity) {
		...
		if (!tmp) {
			wmiiv_log(SWAY_ERROR, "Unable to allocate memory for pollfd");
			return;
		}
		loop->fds = tmp;
...
struct loop_timer *loop_add_timer(struct loop *loop, int ms,
	...
	if (!timer) {
		wmiiv_log(SWAY_ERROR, "Unable to allocate memory for timer");
		return NULL;
	}
