...
struct seat_config *new_seat_config(const char* name) {
	...
	if (!seat) {
		wmiiv_log(WMIIV_DEBUG, "Unable to allocate seat config");
		return NULL;
	}
...
static void merge_wildcard_on_all(struct seat_config *wildcard) {
	for (int i = 0; i < config->seat_configs->length; i++) {
		...
		if (strcmp(wildcard->name, sc->name) != 0) {
			wmiiv_log(WMIIV_DEBUG, "Merging seat * config on %s", sc->name);
...
struct seat_config *store_seat_config(struct seat_config *sc) {
	...
	if (i >= 0) {
		wmiiv_log(WMIIV_DEBUG, "Merging on top of existing seat config");
		struct seat_config *current = config->seat_configs->items[i];
		merge_seat_config(current, sc);
		free_seat_config(sc);
		sc = current;
	} else if (!wildcard) {
		wmiiv_log(WMIIV_DEBUG, "Adding non-wildcard seat config");
		i = list_seq_find(config->seat_configs, seat_name_cmp, "*");
		if (i >= 0) {
			wmiiv_log(WMIIV_DEBUG, "Merging on top of seat * config");
		...
		wmiiv_log(WMIIV_DEBUG, "Adding seat * config");
		list_add(config->seat_configs, sc);
	}

	wmiiv_log(WMIIV_DEBUG, "Config stored for seat %s", sc->name);
...
struct seat_attachment_config *seat_attachment_config_new(void) {
	...
	if (!attachment) {
		wmiiv_log(WMIIV_DEBUG, "cannot allocate attachment config");
		return NULL;
	}
	return attachment;
}
