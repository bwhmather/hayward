...
struct input_config *new_input_config(const char* identifier) {
	...
	if (!input) {
		wmiiv_log(WMIIV_DEBUG, "Unable to allocate input config");
		return NULL;
	}
	wmiiv_log(WMIIV_DEBUG, "new_input_config(%s)", identifier);
...
static bool validate_wildcard_on_all(struct input_config *wildcard,
	...
	for (int i = 0; i < config->input_configs->length; i++) {
		...
		if (strcmp(wildcard->identifier, ic->identifier) != 0) {
			wmiiv_log(WMIIV_DEBUG, "Validating xkb merge of * on %s",
					ic->identifier);
	...
	for (int i = 0; i < config->input_type_configs->length; i++) {
		...
		wmiiv_log(WMIIV_DEBUG, "Validating xkb merge of * config on %s",
				ic->identifier);
		if (!validate_xkb_merge(ic, wildcard, error)) {
			return false;
		}
...
static void merge_wildcard_on_all(struct input_config *wildcard) {
	for (int i = 0; i < config->input_configs->length; i++) {
		struct input_config *ic = config->input_configs->items[i];
		if (strcmp(wildcard->identifier, ic->identifier) != 0) {
			wmiiv_log(WMIIV_DEBUG, "Merging input * config on %s", ic->identifier);
...
static bool validate_type_on_existing(struct input_config *type_wildcard,
	...
	for (int i = 0; i < config->input_configs->length; i++) {
		...
		if (strcmp(ic->input_type, type_wildcard->identifier + 5) == 0) {
			wmiiv_log(WMIIV_DEBUG, "Validating merge of %s on %s",
...
static void merge_type_on_existing(struct input_config *type_wildcard) {
	for (int i = 0; i < config->input_configs->length; i++) {
		...
		if (strcmp(ic->input_type, type_wildcard->identifier + 5) == 0) {
			wmiiv_log(WMIIV_DEBUG, "Merging %s top of %s",
...
struct input_config *store_input_config(struct input_config *ic,
	...
	wmiiv_log(WMIIV_DEBUG, "Config stored for input %s", ic->identifier);
