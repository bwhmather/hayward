...
static void arrange_layer(struct wmiiv_output *output, struct wl_list *list,
		struct wlr_box *usable_area, bool exclusive) {
	struct wmiiv_layer_surface *wmiiv_layer;
	struct wlr_box full_area = { 0 };
	wlr_output_effective_resolution(output->wlr_output,
			&full_area.width, &full_area.height);
	wl_list_for_each(wmiiv_layer, list, link) {
		struct wlr_layer_surface_v1 *layer = wmiiv_layer->layer_surface;
		...
		if (!wmiiv_assert(box.width >= 0 && box.height >= 0,
...
void arrange_layers(struct wmiiv_output *output) {
	...
	struct wmiiv_layer_surface *layer, *topmost = NULL;
...
static struct wmiiv_layer_surface *find_mapped_layer_by_client(
	...
	for (int i = 0; i < root->outputs->length; ++i) {
		...
		struct wmiiv_layer_surface *lsurface;
...
static void handle_output_destroy(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_surface *wmiiv_layer =
		wl_container_of(listener, wmiiv_layer, output_destroy);
	...
	struct wl_client *client =
		wl_resource_get_client(wmiiv_layer->layer_surface->resource);
	bool set_focus = seat->exclusive_client == client;

	if (set_focus) {
		struct wmiiv_layer_surface *layer =
			find_mapped_layer_by_client(client, wmiiv_layer->layer_surface->output);
		if (layer) {
			seat_set_focus_layer(seat, layer->layer_surface);
		}
	}

	wlr_layer_surface_v1_destroy(wmiiv_layer->layer_surface);
}
...
static void handle_surface_commit(struct wl_listener *listener, void *data) {
	...
	wmiiv_assert(wlr_output, "wlr_layer_surface_v1 has null output");
...
static void unmap(struct wmiiv_layer_surface *wmiiv_layer) {
	struct wmiiv_seat *seat;
	wl_list_for_each(seat, &server.input->seats, link) {
		if (seat->focused_layer == wmiiv_layer->layer_surface) {
			seat_set_focus_layer(seat, NULL);
		}
	}
	...
	struct wlr_output *wlr_output = wmiiv_layer->layer_surface->output;
	wmiiv_assert(wlr_output, "wlr_layer_surface_v1 has null output");
	struct wmiiv_output *output = wlr_output->data;
	output_damage_surface(output, wmiiv_layer->geo.x, wmiiv_layer->geo.y,
		wmiiv_layer->layer_surface->surface, true);
}

static void layer_subsurface_destroy(struct wmiiv_layer_subsurface *subsurface);
...
static void handle_destroy(struct wl_listener *listener, void *data) {
	...
	struct wmiiv_layer_subsurface *subsurface, *subsurface_tmp;
	wl_list_for_each_safe(subsurface, subsurface_tmp, &wmiiv_layer->subsurfaces, link) {
		layer_subsurface_destroy(subsurface);
	}

	wl_list_remove(&wmiiv_layer->link);
	wl_list_remove(&wmiiv_layer->destroy.link);
	wl_list_remove(&wmiiv_layer->map.link);
	wl_list_remove(&wmiiv_layer->unmap.link);
	wl_list_remove(&wmiiv_layer->surface_commit.link);
	wl_list_remove(&wmiiv_layer->new_popup.link);
	wl_list_remove(&wmiiv_layer->new_subsurface.link);
	...
	arrange_layers(output);
	transaction_commit_dirty();
	wl_list_remove(&wmiiv_layer->output_destroy.link);
	wmiiv_layer->layer_surface->output = NULL;

	free(wmiiv_layer);
}

static void handle_map(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_surface *wmiiv_layer = wl_container_of(listener,
			wmiiv_layer, map);
	struct wlr_output *wlr_output = wmiiv_layer->layer_surface->output;
	wmiiv_assert(wlr_output, "wlr_layer_surface_v1 has null output");
	struct wmiiv_output *output = wlr_output->data;
	output_damage_surface(output, wmiiv_layer->geo.x, wmiiv_layer->geo.y,
		wmiiv_layer->layer_surface->surface, true);
	wlr_surface_send_enter(wmiiv_layer->layer_surface->surface,
		wmiiv_layer->layer_surface->output);
	cursor_rebase_all();
}

static void handle_unmap(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_surface *wmiiv_layer = wl_container_of(
			listener, wmiiv_layer, unmap);
	unmap(wmiiv_layer);
}

static void subsurface_damage(struct wmiiv_layer_subsurface *subsurface,
		bool whole) {
	struct wmiiv_layer_surface *layer = subsurface->layer_surface;
...
static void subsurface_handle_unmap(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_subsurface *subsurface =
...
static struct wmiiv_layer_subsurface *create_subsurface(
		struct wlr_subsurface *wlr_subsurface,
		struct wmiiv_layer_surface *layer_surface) {
	struct wmiiv_layer_subsurface *subsurface =
			calloc(1, sizeof(struct wmiiv_layer_subsurface));
	if (subsurface == NULL) {
		return NULL;
	}
...
static void handle_new_subsurface(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_surface *wmiiv_layer_surface =
			wl_container_of(listener, wmiiv_layer_surface, new_subsurface);
	struct wlr_subsurface *wlr_subsurface = data;
	create_subsurface(wlr_subsurface, wmiiv_layer_surface);
}


static struct wmiiv_layer_surface *popup_get_layer(
		struct wmiiv_layer_popup *popup) {
...
static void popup_damage(struct wmiiv_layer_popup *layer_popup, bool whole) {
...
static void popup_handle_map(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_popup *popup = wl_container_of(listener, popup, map);
	struct wmiiv_layer_surface *layer = popup_get_layer(popup);
	struct wlr_output *wlr_output = layer->layer_surface->output;
	wmiiv_assert(wlr_output, "wlr_layer_surface_v1 has null output");
...
static void popup_handle_unmap(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_popup *popup = wl_container_of(listener, popup, unmap);
	popup_damage(popup, true);
}

static void popup_handle_commit(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_popup *popup = wl_container_of(listener, popup, commit);
...
static void popup_unconstrain(struct wmiiv_layer_popup *popup) {
	struct wmiiv_layer_surface *layer = popup_get_layer(popup);
...
static struct wmiiv_layer_popup *create_popup(struct wlr_xdg_popup *wlr_popup,
		enum layer_parent parent_type, void *parent) {
	struct wmiiv_layer_popup *popup =
		calloc(1, sizeof(struct wmiiv_layer_popup));
	if (popup == NULL) {
		return NULL;
	}
...
static void popup_handle_new_popup(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_popup *wmiiv_layer_popup =
		wl_container_of(listener, wmiiv_layer_popup, new_popup);
	struct wlr_xdg_popup *wlr_popup = data;
	create_popup(wlr_popup, LAYER_PARENT_POPUP, wmiiv_layer_popup);
}

static void handle_new_popup(struct wl_listener *listener, void *data) {
	struct wmiiv_layer_surface *wmiiv_layer_surface =
		wl_container_of(listener, wmiiv_layer_surface, new_popup);
	struct wlr_xdg_popup *wlr_popup = data;
	create_popup(wlr_popup, LAYER_PARENT_LAYER, wmiiv_layer_surface);
}
...
void handle_layer_shell_surface(struct wl_listener *listener, void *data) {
	...
	if (!layer_surface->output) {
		...
		struct wmiiv_output *output = NULL;
		struct wmiiv_seat *seat = input_manager_get_default_seat();
		if (seat) {
	...
	struct wmiiv_layer_surface *wmiiv_layer =
		calloc(1, sizeof(struct wmiiv_layer_surface));
	if (!wmiiv_layer) {
		return;
	}

	wl_list_init(&wmiiv_layer->subsurfaces);

	wmiiv_layer->surface_commit.notify = handle_surface_commit;
	wl_signal_add(&layer_surface->surface->events.commit,
		&wmiiv_layer->surface_commit);

	wmiiv_layer->destroy.notify = handle_destroy;
	wl_signal_add(&layer_surface->events.destroy, &wmiiv_layer->destroy);
	wmiiv_layer->map.notify = handle_map;
	wl_signal_add(&layer_surface->events.map, &wmiiv_layer->map);
	wmiiv_layer->unmap.notify = handle_unmap;
	wl_signal_add(&layer_surface->events.unmap, &wmiiv_layer->unmap);
	wmiiv_layer->new_popup.notify = handle_new_popup;
	wl_signal_add(&layer_surface->events.new_popup, &wmiiv_layer->new_popup);
	wmiiv_layer->new_subsurface.notify = handle_new_subsurface;
	wl_signal_add(&layer_surface->surface->events.new_subsurface,
			&wmiiv_layer->new_subsurface);

	wmiiv_layer->layer_surface = layer_surface;
	layer_surface->data = wmiiv_layer;

	struct wmiiv_output *output = layer_surface->output->data;
	wmiiv_layer->output_destroy.notify = handle_output_destroy;
	wl_signal_add(&output->events.disable, &wmiiv_layer->output_destroy);
