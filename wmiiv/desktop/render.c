...
static void render_surface_iterator(struct wmiiv_output *output,
...
static void render_layer_toplevel(struct wmiiv_output *output,
...
static void render_layer_popups(struct wmiiv_output *output,
		pixman_region32_t *damage, struct wl_list *layer_surfaces) {
	struct render_data data = {
		.damage = damage,
		.alpha = 1.0f,
...
#if HAVE_XWAYLAND
static void render_unmanaged(struct wmiiv_output *output,
...
static void render_drag_icons(struct wmiiv_output *output,
...
static void render_view_toplevels(struct wmiiv_view *view,
		struct wmiiv_output *output, pixman_region32_t *damage, float alpha) {
	struct render_data data = {
		.damage = damage,
		.alpha = alpha,
	};
...
static void render_view_popups(struct wmiiv_view *view,
		struct wmiiv_output *output, pixman_region32_t *damage, float alpha) {
	struct render_data data = {
		.damage = damage,
		.alpha = alpha,
...
static void render_saved_view(struct wmiiv_view *view,
		struct wmiiv_output *output, pixman_region32_t *damage, float alpha) {
	struct wlr_output *wlr_output = output->wlr_output;
	...
	// as sampled here.
	// https://github.com/wmiivwm/wmiiv/pull/4465#discussion_r321082059
}
...
static void render_view(struct wmiiv_output *output, pixman_region32_t *damage,
	...
	enum wmiiv_column_layout layout = window_current_parent_layout(window);
...
static void render_titlebar(struct wmiiv_output *output,
		pixman_region32_t *output_damage, struct wmiiv_window *window,
...
static void render_top_border(struct wmiiv_output *output,
		pixman_region32_t *output_damage, struct wmiiv_window *container,
		struct border_colors *colors) {
	struct wmiiv_window_state *state = &container->current;
	if (!state->border_top) {
		return;
	}
	struct wlr_box box;
...
static void render_column_split(struct wmiiv_output *output, pixman_region32_t *damage, struct wmiiv_column *column) {
	...
	for (int i = 0; i < column->current.children->length; ++i) {
		...
		struct wlr_texture *marks_texture;
		struct wmiiv_window_state *state = &child->current;
...
static void render_floating_window(struct wmiiv_output *soutput, pixman_region32_t *damage, struct wmiiv_window *window) {
	struct wmiiv_view *view = window->view;
	struct border_colors *colors;
	struct wlr_texture *title_texture;
	struct wlr_texture *marks_texture;
	...
	for (int i = 0; i < root->outputs->length; ++i) {
		...
		for (int j = 0; j < output->current.workspaces->length; ++j) {
			...
			for (int k = 0; k < workspace->current.floating->length; ++k) {
				struct wmiiv_window *floater = workspace->current.floating->items[k];
...
static void render_seatops(struct wmiiv_output *output,
		pixman_region32_t *damage) {
...
void output_render(struct wmiiv_output *output, struct timespec *when,
	...
	struct wmiiv_window *fullscreen_container = root->fullscreen_global;
