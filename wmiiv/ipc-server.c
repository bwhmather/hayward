...
int ipc_handle_connection(int fd, uint32_t mask, void *data) {
	...
	wmiiv_log(SWAY_DEBUG, "Event on IPC listening socket");
	assert(mask == WL_EVENT_READABLE);

	int client_fd = accept(ipc_socket, NULL, NULL);
	if (client_fd == -1) {
		wmiiv_log_errno(SWAY_ERROR, "Unable to accept IPC client connection");
		return 0;
	}
	...
	if ((flags = fcntl(client_fd, F_GETFD)) == -1
		...
		wmiiv_log_errno(SWAY_ERROR, "Unable to set CLOEXEC on IPC client socket");
		close(client_fd);
		return 0;
	}
	if ((flags = fcntl(client_fd, F_GETFL)) == -1
			|| fcntl(client_fd, F_SETFL, flags|O_NONBLOCK) == -1) {
		wmiiv_log_errno(SWAY_ERROR, "Unable to set NONBLOCK on IPC client socket");
		close(client_fd);
		return 0;
	}
	...
	if (!client) {
		wmiiv_log(SWAY_ERROR, "Unable to allocate ipc client");
		close(client_fd);
		return 0;
	}
	...
	if (!client->write_buffer) {
		wmiiv_log(SWAY_ERROR, "Unable to allocate ipc client write buffer");
		close(client_fd);
		return 0;
	}

	wmiiv_log(SWAY_DEBUG, "New client: fd %d", client_fd);
...
int ipc_client_handle_readable(int client_fd, uint32_t mask, void *data) {
	...
	if (mask & WL_EVENT_ERROR) {
		wmiiv_log(SWAY_ERROR, "IPC Client socket error, removing client");
		ipc_client_disconnect(client);
		return 0;
	}

	if (mask & WL_EVENT_HANGUP) {
		wmiiv_log(SWAY_DEBUG, "Client %d hung up", client->fd);
		ipc_client_disconnect(client);
		return 0;
	}

	wmiiv_log(SWAY_DEBUG, "Client %d readable", client->fd);

	int read_available;
	if (ioctl(client_fd, FIONREAD, &read_available) == -1) {
		wmiiv_log_errno(SWAY_INFO, "Unable to read IPC socket buffer size");
		ipc_client_disconnect(client);
		return 0;
	}
	...
	if (received == -1) {
		wmiiv_log_errno(SWAY_INFO, "Unable to receive header from IPC client");
		ipc_client_disconnect(client);
		return 0;
	}

	if (memcmp(buf, ipc_magic, sizeof(ipc_magic)) != 0) {
		wmiiv_log(SWAY_DEBUG, "IPC header check failed");
		ipc_client_disconnect(client);
		return 0;
	}
...
static void ipc_send_event(const char *json_string, enum ipc_command_type event) {
	...
	for (int i = 0; i < ipc_client_list->length; i++) {
		...
		if (!ipc_send_reply(client, event, json_string,
			...
			wmiiv_log_errno(SWAY_INFO, "Unable to send reply to IPC client");
...
void ipc_event_workspace(struct wmiiv_workspace *old,
	...
	wmiiv_log(SWAY_DEBUG, "Sending workspace::%s event", change);
...
void ipc_event_window(struct wmiiv_container *window, const char *change) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending window::%s event", change);
	json_object *obj = json_object_new_object();
	json_object_object_add(obj, "change", json_object_new_string(change));
...
void ipc_event_barconfig_update(struct bar_config *bar) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending barconfig_update event");
...
void ipc_event_bar_state_update(struct bar_config *bar) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending bar_state_update event");
...
void ipc_event_mode(const char *mode, bool pango) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending mode::%s event", mode);
	json_object *obj = json_object_new_object();
...
void ipc_event_shutdown(const char *reason) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending shutdown::%s event", reason);
...
void ipc_event_binding(struct wmiiv_binding *binding) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending binding event");
	...
	default:
		wmiiv_log(SWAY_DEBUG, "Unsupported ipc binding event");
...
static void ipc_event_tick(const char *payload) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending tick event");
...
void ipc_event_input(const char *change, struct wmiiv_input_device *device) {
	...
	wmiiv_log(SWAY_DEBUG, "Sending input event");
...
int ipc_client_handle_writable(int client_fd, uint32_t mask, void *data) {
	...
	wmiiv_log(SWAY_DEBUG, "Client %d writable", client->fd);
	...
	if (written == -1 && errno == EAGAIN) {
		...
		wmiiv_log_errno(SWAY_INFO, "Unable to send data from queue to IPC client");
		ipc_client_disconnect(client);
		return 0;
	}
...
void ipc_client_disconnect(struct ipc_client *client) {
	...
	wmiiv_log(SWAY_INFO, "IPC Client %d disconnected", client->fd);
...
void ipc_client_handle_command(struct ipc_client *client, uint32_t payload_length,
	...
	if (!buf) {
		wmiiv_log_errno(SWAY_INFO, "Unable to allocate IPC payload");
		ipc_client_disconnect(client);
		return;
	}
	if (payload_length > 0) {
		...
		if (received == -1)
		{
			wmiiv_log_errno(SWAY_INFO, "Unable to receive payload from IPC client");
			ipc_client_disconnect(client);
			free(buf);
			return;
		}
	...
	case IPC_SUBSCRIBE:
	{
		...
		if (request == NULL || !json_object_is_type(request, json_type_array)) {
			...
			wmiiv_log(SWAY_INFO, "Failed to parse subscribe request");
			goto exit_cleanup;
		}
		...
		for (size_t i = 0; i < json_object_array_length(request); i++) {
			...
			if (strcmp(event_type, "workspace") == 0) {
				...
				wmiiv_log(SWAY_INFO, "Unsupported event type in subscribe request");
				goto exit_cleanup;
			}
		}
	...
	default:
		wmiiv_log(SWAY_INFO, "Unknown IPC command type %x", payload_type);
		goto exit_cleanup;
	}
...
bool ipc_send_reply(struct ipc_client *client, enum ipc_command_type payload_type,
	...
	if (client->write_buffer_size > 4e6) { // 4 MB
		wmiiv_log(SWAY_ERROR, "Client write buffer too big (%zu), disconnecting client",
				client->write_buffer_size);
		ipc_client_disconnect(client);
		return false;
	}
	...
	if (!new_buffer) {
		wmiiv_log(SWAY_ERROR, "Unable to reallocate ipc client write buffer");
		ipc_client_disconnect(client);
		return false;
	}
	...
	wmiiv_log(SWAY_DEBUG, "Added IPC reply of type 0x%x to client %d queue: %s",
