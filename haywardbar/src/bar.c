...
static void haywardbar_output_free(struct haywardbar_output *output) {
	if (!output) {
		return;
	}
	hayward_log(HAYWARD_DEBUG, "Removing output %s", output->name);
...
static void set_output_dirty(struct haywardbar_output *output) {
...
static void add_layer_surface(struct haywardbar_output *output) {
...
static bool bar_uses_output(struct haywardbar_output *output) {
...
static void xdg_output_handle_description(void *data,
	...
	if (paren) {
		...
		if (!output->identifier) {
			hayward_log(HAYWARD_ERROR, "Failed to allocate output identifier");
			return;
		}
...
static void add_xdg_output(struct haywardbar_output *output) {
...
static void display_in(int fd, short mask, void *data) {
	...
	if (mask & (POLLHUP | POLLERR)) {
		if (mask & POLLERR) {
			hayward_log(HAYWARD_ERROR, "Wayland display poll error");
		}
		bar->running = false;
		return;
	}
	if (wl_display_dispatch(bar->display) == -1) {
		hayward_log(HAYWARD_ERROR, "wl_display_dispatch failed");
		bar->running = false;
	}
}
...
static void ipc_in(int fd, short mask, void *data) {
	...
	if (mask & (POLLHUP | POLLERR)) {
		if (mask & POLLERR) {
			hayward_log(HAYWARD_ERROR, "IPC poll error");
		}
		bar->running = false;
		return;
	}
