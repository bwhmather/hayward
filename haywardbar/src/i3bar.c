...
static bool i3bar_parse_json_color(json_object *json, uint32_t *color) {
	...
	if (!color_set) {
		hayward_log(HAYWARD_ERROR, "Ignoring invalid block hexadecimal color string: %s", hexstring);
	}
	return color_set;
}
	...
	while (!status->started) { // look for opening bracket
		for (size_t c = 0; c < status->buffer_index; ++c) {
			if (status->buffer[c] == '[') {
				...
				hayward_log(HAYWARD_DEBUG, "Invalid i3bar json: expected '[' but encountered '%c'",
						status->buffer[c]);
		...
		if (status->expecting_comma) {
			...
			if (err == json_tokener_success) {
				...
				*last_char_pos = '\0';
				...
				hayward_log(HAYWARD_DEBUG, "Received i3bar json: '%s%c'",
				...
				*last_char_pos = last_char;
				...
				hayward_log(HAYWARD_DEBUG, "Failed to parse i3bar json - %s: '%s%c'",
