...
static void
i3bar_parse_json(struct status_line *status, struct json_object *json_array) {
    ...
    while (true) {
        ...
        if (status->expecting_comma) {
            ...
            if (err == json_tokener_success) {
                ...
                // the last character is temporarily replaced with a null
        ...
        ssize_t read_bytes = read(
            status->read_fd, &status->buffer[status->buffer_index],
