...
static int
read_pixmap(sd_bus_message *msg, struct haywardbar_sni *sni, const char *prop, list_t **dest) {
    int ret = sd_bus_message_enter_container(msg, 'a', "(iiay)");
    if (ret < 0) {
        hwd_log(HWD_ERROR, "%s %s: %s", sni->watcher_id, prop, strerror(-ret));
        return ret;
    }

    if (sd_bus_message_at_end(msg, 0)) {
        hwd_log(HWD_DEBUG, "%s %s no. of icons = 0", sni->watcher_id, prop);
        return ret;
    }
    ...
    while (!sd_bus_message_at_end(msg, 0)) {
        ...
        if (height > 0 && width == height) {
            hwd_log(HWD_DEBUG, "%s %s: found icon w:%d h:%d", sni->watcher_id, prop, width, height);
            struct haywardbar_pixmap *pixmap = malloc(sizeof(struct haywardbar_pixmap) + npixels);
            pixmap->size = height;
            ...
            hwd_log(
                HWD_DEBUG, "%s %s: discard invalid icon w:%d h:%d", sni->watcher_id, prop, width,
                height
            );
        }
    ...
    *dest = pixmaps;
    hwd_log(HWD_DEBUG, "%s %s no. of icons = %d", sni->watcher_id, prop, pixmaps->length);
...
get_property_callback(sd_bus_message *msg, void *data, sd_bus_error *error) {
    struct haywardbar_sni_slot *d = data;
    struct haywardbar_sni *sni = d->sni;
    ...
    if (sd_bus_message_is_method_error(msg, NULL)) {
        ...
        hwd_log_importance_t log_lv = HWD_ERROR;
        if ((!strcmp(prop, "IconThemePath")) &&
            (!strcmp(err->name, SD_BUS_ERROR_UNKNOWN_PROPERTY))) {
            log_lv = HWD_DEBUG;
        }
        hwd_log(log_lv, "%s %s: %s", sni->watcher_id, prop, err->message);
    ...
    if (!type) {
        ...
        if (*type == 's' || *type == 'o') {
            char **str = dest;
            *str = strdup(*str);
            hwd_log(HWD_DEBUG, "%s %s = '%s'", sni->watcher_id, prop, *str);
        } else if (*type == 'b') {
            hwd_log(
                HWD_DEBUG, "%s %s = %s", sni->watcher_id, prop, *(bool *)dest ? "true" : "false"
            );
        }
    }
...
static void
sni_get_property_async(struct haywardbar_sni *sni, const char *prop, const char *type, void *dest) {
    struct haywardbar_sni_slot *data = calloc(1, sizeof(struct haywardbar_sni_slot));
    data->sni = sni;
    data->prop = prop;
    data->type = type;
    data->dest = dest;
    int ret = sd_bus_call_method_async(
        sni->tray->bus, &data->slot, sni->service, sni->path, "org.freedesktop.DBus.Properties",
 ...
 */
static int
sni_check_msg_sender(struct haywardbar_sni *sni, sd_bus_message *msg, const char *signal) {
    ...
    if (sni->service[0] == ':' || has_well_known_names) {
        hwd_log(HWD_DEBUG, "%s has new %s", sni->watcher_id, signal);
        return 1;
    } else {
        hwd_log(HWD_DEBUG, "%s may have new %s", sni->watcher_id, signal);
        return 0;
    }
}
...
handle_new_status(sd_bus_message *msg, void *data, sd_bus_error *error) {
    ...
    if (ret == 1) {
        ...
        if (r < 0) {
            hwd_log(HWD_ERROR, "%s new status error: %s", sni->watcher_id, strerror(-ret));
            ret = r;
        } else {
            free(sni->status);
            sni->status = strdup(status);
            hwd_log(HWD_DEBUG, "%s has new status = '%s'", sni->watcher_id, status);
            set_sni_dirty(sni);
        }
    } else {
...
sni_match_signal_async(
    ...
    struct haywardbar_sni_slot *slot = calloc(1, sizeof(struct haywardbar_sni_slot));
    int ret = sd_bus_match_signal_async(
        sni->tray->bus, &slot->slot, sni->service, sni->path, sni->interface, signal, callback,
        NULL, sni
    );
    if (ret >= 0) {
        ...
        hwd_log(
            HWD_ERROR, "%s failed to subscribe to signal %s: %s", sni->service, signal,
            strerror(-ret)
        );
        free(slot);
    }
...
destroy_sni(struct haywardbar_sni *sni) {
    ...
    struct haywardbar_sni_slot *slot, *slot_tmp;
...
static void
handle_click(struct haywardbar_sni *sni, int x, int y, uint32_t button, int delta) {
    ...
    if (!method) {
        static const char *default_bindings[10] = {
            "nop",        "Activate",   "SecondaryActivate", "ContextMenu", "ScrollUp",
            "ScrollDown", "ScrollLeft", "ScrollRight",       "nop",         "nop"};
    ...
    if (strncmp(method, "Scroll", strlen("Scroll")) == 0) {
        ...
        sd_bus_call_method_async(
            sni->tray->bus, NULL, sni->service, sni->path, sni->interface, "Scroll", NULL, NULL,
            "is", delta * sign, orientation
        );
    } else {
        sd_bus_call_method_async(
            sni->tray->bus, NULL, sni->service, sni->path, sni->interface, method, NULL, NULL, "ii",
            x, y
        );
    }
}
...
icon_hotspot_callback(
    ...
    hwd_log(HWD_DEBUG, "Clicked on %s", (char *)data);

    struct haywardbar_tray *tray = output->bar->tray;
    int idx = list_seq_find(tray->items, cmp_sni_id, data);

    if (idx != -1) {
        struct haywardbar_sni *sni = tray->items->items[idx];
        // guess global position since wayland doesn't expose it
        struct haywardbar_config *config = tray->bar->config;
        int global_x = output->output_x + config->gaps.left + x;
        bool top_bar = config->position & ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP;
        int global_y = output->output_y +
            (top_bar ? config->gaps.top + y : (int)output->output_height - config->gaps.bottom - y);

        hwd_log(HWD_DEBUG, "Guessing click position at (%d, %d)", global_x, global_y);
        handle_click(sni, global_x, global_y, button, 1); // TODO get delta from event
        return HOTSPOT_IGNORE;
    } else {
        hwd_log(HWD_DEBUG, "but it doesn't exist");
    }
...
static void
reload_sni(struct haywardbar_sni *sni, char *icon_theme, int target_size) {
    char *icon_name = sni->status[0] == 'N' ? sni->attention_icon_name : sni->icon_name;
    if (icon_name) {
        ...
        char *icon_path = find_icon(
            sni->tray->themes, icon_search_paths, icon_name, target_size, icon_theme,
    ...
    list_t *pixmaps = sni->status[0] == 'N' ? sni->attention_icon_pixmap : sni->icon_pixmap;
    if (pixmaps) {
        struct haywardbar_pixmap *pixmap = NULL;
        int min_error = INT_MAX;
        for (int i = 0; i < pixmaps->length; ++i) {
            struct haywardbar_pixmap *p = pixmaps->items[i];
...
uint32_t
render_sni(
    cairo_t *cairo, struct haywardbar_output *output, double *x, struct haywardbar_sni *sni
) {
    ...
    if (sni->icon) {
        ...
        icon_size =
            actual_size < target_size ? actual_size * (target_size / actual_size) : target_size;
