...
static int read_pixmap(sd_bus_message *msg, struct haywardbar_sni *sni,
		const char *prop, list_t **dest) {
	int ret = sd_bus_message_enter_container(msg, 'a', "(iiay)");
	if (ret < 0) {
		hayward_log(HAYWARD_ERROR, "%s %s: %s", sni->watcher_id, prop, strerror(-ret));
		return ret;
	}

	if (sd_bus_message_at_end(msg, 0)) {
		hayward_log(HAYWARD_DEBUG, "%s %s no. of icons = 0", sni->watcher_id, prop);
		return ret;
	}
	...
	while (!sd_bus_message_at_end(msg, 0)) {
		...
		if (height > 0 && width == height) {
			hayward_log(HAYWARD_DEBUG, "%s %s: found icon w:%d h:%d", sni->watcher_id, prop, width, height);
			...
			hayward_log(HAYWARD_DEBUG, "%s %s: discard invalid icon w:%d h:%d", sni->watcher_id, prop, width, height);
		}
	...
	*dest = pixmaps;
	hayward_log(HAYWARD_DEBUG, "%s %s no. of icons = %d", sni->watcher_id, prop,
			pixmaps->length);
...
static int get_property_callback(sd_bus_message *msg, void *data,
	...
	if (!type) {
		...
		if (*type == 's' || *type == 'o') {
			char **str = dest;
			*str = strdup(*str);
			hayward_log(HAYWARD_DEBUG, "%s %s = '%s'", sni->watcher_id, prop, *str);
		} else if (*type == 'b') {
			hayward_log(HAYWARD_DEBUG, "%s %s = %s", sni->watcher_id, prop,
...
static void sni_get_property_async(struct haywardbar_sni *sni, const char *prop,
 ...
 */
static int sni_check_msg_sender(struct haywardbar_sni *sni, sd_bus_message *msg,
		const char *signal) {
	...
	if (sni->service[0] == ':' || has_well_known_names) {
		...
		hayward_log(HAYWARD_DEBUG, "%s may have new %s", sni->watcher_id, signal);
		return 0;
	}
}
...
static int handle_new_status(sd_bus_message *msg, void *data, sd_bus_error *error) {
	...
	if (ret == 1) {
		...
		if (r < 0) {
			hayward_log(HAYWARD_ERROR, "%s new status error: %s", sni->watcher_id, strerror(-ret));
			ret = r;
		} else {
			free(sni->status);
			sni->status = strdup(status);
			hayward_log(HAYWARD_DEBUG, "%s has new status = '%s'", sni->watcher_id, status);
			set_sni_dirty(sni);
		}
	} else {
...
static void sni_match_signal_async(struct haywardbar_sni *sni, char *signal,
	...
	if (ret >= 0) {
		...
		hayward_log(HAYWARD_ERROR, "%s failed to subscribe to signal %s: %s",
...
static void handle_click(struct haywardbar_sni *sni, int x, int y,
...
static enum hotspot_event_handling icon_hotspot_callback(
	...
	if (idx != -1) {
		...
		hayward_log(HAYWARD_DEBUG, "Guessing click position at (%d, %d)", global_x, global_y);
...
static void reload_sni(struct haywardbar_sni *sni, char *icon_theme,
		int target_size) {
