...
static int read_pixmap(
	...
	if (ret < 0) {
		hayward_log(
			HAYWARD_ERROR, "%s %s: %s", sni->watcher_id, prop, strerror(-ret)
		);
		return ret;
	}
	...
	if (sd_bus_message_at_end(msg, 0)) {
		hayward_log(
			HAYWARD_DEBUG, "%s %s no. of icons = 0", sni->watcher_id, prop
		);
		return ret;
	}
	...
	while (!sd_bus_message_at_end(msg, 0)) {
		...
		if (ret < 0) {
			hayward_log(
				HAYWARD_ERROR, "%s %s: %s", sni->watcher_id, prop,
				strerror(-ret)
			);
			goto error;
		}
		...
		if (height > 0 && width == height) {
			hayward_log(
				HAYWARD_DEBUG, "%s %s: found icon w:%d h:%d", sni->watcher_id,
				prop, width, height
			);
			struct haywardbar_pixmap *pixmap =
				malloc(sizeof(struct haywardbar_pixmap) + npixels);
			pixmap->size = height;

			// convert from network byte order to host byte order
			for (int i = 0; i < height * width; ++i) {
				((uint32_t *)pixmap->pixels)[i] =
					ntohl(((uint32_t *)pixels)[i]);
			}

			list_add(pixmaps, pixmap);
		} else {
			hayward_log(
				HAYWARD_DEBUG, "%s %s: discard invalid icon w:%d h:%d",
	...
	*dest = pixmaps;
	hayward_log(
		HAYWARD_DEBUG, "%s %s no. of icons = %d", sni->watcher_id, prop,
		pixmaps->length
	);
...
get_property_callback(sd_bus_message *msg, void *data, sd_bus_error *error) {
	...
	if (sd_bus_message_is_method_error(msg, NULL)) {
		...
		hayward_log_importance_t log_lv = HAYWARD_ERROR;
		if ((!strcmp(prop, "IconThemePath")) &&
			(!strcmp(err->name, SD_BUS_ERROR_UNKNOWN_PROPERTY))) {
			log_lv = HAYWARD_DEBUG;
		}
		hayward_log(log_lv, "%s %s: %s", sni->watcher_id, prop, err->message);
	...
	if (!type) {
		...
		if (*type == 's' || *type == 'o') {
			...
			*str = strdup(*str);
			hayward_log(
				HAYWARD_DEBUG, "%s %s = '%s'", sni->watcher_id, prop, *str
			);
		} else if (*type == 'b') {
			hayward_log(
				HAYWARD_DEBUG, "%s %s = %s", sni->watcher_id, prop,
	...
	if (strcmp(prop, "Status") == 0 ||
		(sni->status &&
		 (sni->status[0] == 'N' ? prop[0] == 'A' : strncmp(prop, "Icon", 4) == 0
		 ))) {
		set_sni_dirty(sni);
	}
cleanup:
...
static void sni_get_property_async(
	...
	struct haywardbar_sni_slot *data =
		calloc(1, sizeof(struct haywardbar_sni_slot));
	data->sni = sni;
	data->prop = prop;
	data->type = type;
	data->dest = dest;
...
static int sni_check_msg_sender(
	...
	if (sni->service[0] == ':' || has_well_known_names) {
		hayward_log(HAYWARD_DEBUG, "%s has new %s", sni->watcher_id, signal);
		return 1;
	} else {
		hayward_log(
			HAYWARD_DEBUG, "%s may have new %s", sni->watcher_id, signal
		);
		return 0;
	}
}
...
handle_new_icon(sd_bus_message *msg, void *data, sd_bus_error *error) {
	...
	if (!strcmp(sni->interface, "org.kde.StatusNotifierItem")) {
		sni_get_property_async(
			sni, "IconThemePath", "s", &sni->icon_theme_path
		);
	}
...
static int handle_new_attention_icon(
	...
	struct haywardbar_sni *sni = data;
	sni_get_property_async(
		sni, "AttentionIconName", "s", &sni->attention_icon_name
	);
	sni_get_property_async(
		sni, "AttentionIconPixmap", NULL, &sni->attention_icon_pixmap
	);
...
handle_new_status(sd_bus_message *msg, void *data, sd_bus_error *error) {
	...
	if (ret == 1) {
		...
		if (r < 0) {
			hayward_log(
				HAYWARD_ERROR, "%s new status error: %s", sni->watcher_id,
				strerror(-ret)
			);
			ret = r;
		} else {
			...
			hayward_log(
				HAYWARD_DEBUG, "%s has new status = '%s'", sni->watcher_id,
				status
			);
			set_sni_dirty(sni);
		}
...
static void sni_match_signal_async(
	...
	int ret = sd_bus_match_signal_async(
		sni->tray->bus, &slot->slot, sni->service, sni->path, sni->interface,
		signal, callback, NULL, sni
	);
	if (ret >= 0) {
		wl_list_insert(&sni->slots, &slot->link);
	} else {
		hayward_log(
			HAYWARD_ERROR, "%s failed to subscribe to signal %s: %s",
...
static void handle_click(
	...
	if (strncmp(method, "Scroll", strlen("Scroll")) == 0) {
		...
		sd_bus_call_method_async(
			sni->tray->bus, NULL, sni->service, sni->path, sni->interface,
			"Scroll", NULL, NULL, "is", delta * sign, orientation
		);
	} else {
		sd_bus_call_method_async(
			sni->tray->bus, NULL, sni->service, sni->path, sni->interface,
...
static enum hotspot_event_handling icon_hotspot_callback(
	...
	hayward_log(HAYWARD_DEBUG, "Clicked on %s", (char *)data);
	...
	if (idx != -1) {
		...
		int global_y = output->output_y +
			(top_bar ? config->gaps.top + y
					 : (int)output->output_height - config->gaps.bottom - y);

		hayward_log(
			HAYWARD_DEBUG, "Guessing click position at (%d, %d)", global_x,
			global_y
		);
		handle_click(
			sni, global_x, global_y, button, 1
		); // TODO get delta from event
		return HOTSPOT_IGNORE;
	} else {
		hayward_log(HAYWARD_DEBUG, "but it doesn't exist");
	}
...
reload_sni(struct haywardbar_sni *sni, char *icon_theme, int target_size) {
	...
	if (icon_name) {
		...
		char *icon_path = find_icon(
			sni->tray->themes, icon_search_paths, icon_name, target_size,
...
uint32_t render_sni(
	...
	if (sni->icon) {
		...
		icon = cairo_image_surface_create(
	...
	*x -= size;
	...
	cairo_matrix_translate(
		&scale_matrix, -(*x + descaled_padding), -(icon_y + descaled_padding)
	);
