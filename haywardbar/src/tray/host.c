...
add_sni(struct haywardbar_tray *tray, char *id) {
    ...
    if (idx == -1) {
        hwd_log(HWD_INFO, "Registering Status Notifier Item '%s'", id);
...
handle_sni_unregistered(sd_bus_message *msg, void *data, sd_bus_error *error) {
    ...
    int ret = sd_bus_message_read(msg, "s", &id);
    if (ret < 0) {
        hwd_log(HWD_ERROR, "Failed to parse unregister SNI message: %s", strerror(-ret));
    }

    struct haywardbar_tray *tray = data;
    int idx = list_seq_find(tray->items, cmp_sni_id, id);
    if (idx != -1) {
        hwd_log(HWD_INFO, "Unregistering Status Notifier Item '%s'", id);
...
get_registered_snis_callback(sd_bus_message *msg, void *data, sd_bus_error *error) {
    if (sd_bus_message_is_method_error(msg, NULL)) {
        ...
        hwd_log(HWD_ERROR, "Failed to get registered SNIs: %s", err->message);
        return -sd_bus_error_get_errno(err);
    }

    int ret = sd_bus_message_enter_container(msg, 'v', NULL);
    if (ret < 0) {
        hwd_log(HWD_ERROR, "Failed to read registered SNIs: %s", strerror(-ret));
        return ret;
    }
...
register_to_watcher(struct haywardbar_host *host) {
    ...
    int ret = sd_bus_call_method_async(
        host->tray->bus, NULL, host->watcher_interface, watcher_path, host->watcher_interface,
        "RegisterStatusNotifierHost", NULL, NULL, "s", host->service
    );
    if (ret < 0) {
        hwd_log(HWD_ERROR, "Failed to send register call: %s", strerror(-ret));
        return false;
    }

    ret = sd_bus_call_method_async(
        host->tray->bus, NULL, host->watcher_interface, watcher_path,
        "org.freedesktop.DBus.Properties", "Get", get_registered_snis_callback, host->tray, "ss",
        host->watcher_interface, "RegisteredStatusNotifierItems"
    );
    if (ret < 0) {
        hwd_log(HWD_ERROR, "Failed to get registered SNIs: %s", strerror(-ret));
    }
...
init_host(struct haywardbar_host *host, char *protocol, struct haywardbar_tray *tray) {
    ...
    int ret = sd_bus_match_signal(
        tray->bus, &reg_slot, host->watcher_interface, watcher_path, host->watcher_interface,
        "StatusNotifierItemRegistered", handle_sni_registered, tray
    );
    if (ret < 0) {
        hwd_log(HWD_ERROR, "Failed to subscribe to registering events: %s", strerror(-ret));
        goto error;
    }
    ret = sd_bus_match_signal(
        tray->bus, &unreg_slot, host->watcher_interface, watcher_path, host->watcher_interface,
    ...
    ret = sd_bus_match_signal(
        tray->bus, &watcher_slot, "org.freedesktop.DBus", "/org/freedesktop/DBus",
    ...
    pid_t pid = getpid();
    size_t service_len = snprintf(NULL, 0, "org.%s.StatusNotifierHost-%d", protocol, pid) + 1;
    ...
    if (ret < 0) {
        hwd_log(HWD_DEBUG, "Failed to acquire service name: %s", strerror(-ret));
        goto error;
    }
    ...
    hwd_log(HWD_DEBUG, "Registered %s", host->service);
    return true;
error:
