    ...
    while (1) {
        ...
        case 'Z': // Button (Dismiss, No Terminal)
            if (haywardnag) {
                ...
                button->type = HAYWARDNAG_ACTION_COMMAND;
                button->action = strdup(argv[optind]);
                button->terminal = c == 'b';
                button->dismiss = c == 'z' || c == 'Z';
                list_add(haywardnag->buttons, button);
            }
            optind++;
            break;
        case 'c': // Config
        ...
        case 'l': // Detailed Message
            if (haywardnag) {
                ...
                haywardnag->details.message = read_and_trim_stdin();
                if (!haywardnag->details.message) {
                    return EXIT_FAILURE;
                }
                haywardnag->details.button_up.text = strdup("▲");
                haywardnag->details.button_down.text = strdup("▼");
            }
            break;
        case 'L': // Detailed Button Text
            if (haywardnag) {
                free(haywardnag->details.button_details.text);
                haywardnag->details.button_details.text = strdup(optarg);
            }
            break;
        case 'm': // Message
            if (haywardnag) {
                free(haywardnag->message);
                haywardnag->message = strdup(optarg);
            }
            break;
        case 'o': // Output
            if (type) {
        ...
        case 't': // Type
            if (haywardnag) {
                haywardnag->type = haywardnag_type_get(types, optarg);
                if (!haywardnag->type) {
...
haywardnag_get_config_path(void) {
    static const char *config_paths[] = {
        ...
        SYSCONFDIR "/haywardnag/config",
    };

    char *config_home = getenv("XDG_CONFIG_HOME");
    if (!config_home || config_home[0] == '\0') {
        config_paths[1] = "$HOME/.config/haywardnag/config";
    }
...
haywardnag_load_config(char *path, struct haywardnag *haywardnag, list_t *types) {
    ...
    struct haywardnag_type *type = haywardnag_type_new("<config>");
    list_add(types, type);
    ...
    while ((nread = getline(&line, &line_size, config)) != -1) {
        ...
        if (line[0] == '[') {
            ...
            *close = '\0';
            type = haywardnag_type_get(types, &line[1]);
            if (!type) {
                type = haywardnag_type_new(&line[1]);
                list_add(types, type);
            }
        } else {
