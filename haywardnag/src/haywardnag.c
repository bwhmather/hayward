...
static bool terminal_execute(char *terminal, char *command) {
	...
	hayward_log_errno(HAYWARD_ERROR, "Failed to run command, execlp() returned.");
	free(cmd);
	return false;
}

static void haywardnag_button_execute(struct haywardnag *haywardnag,
		struct haywardnag_button *button) {
	hayward_log(HAYWARD_DEBUG, "Executing [%s]: %s", button->text, button->action);
...
static void surface_enter(void *data, struct wl_surface *surface,
	...
	wl_list_for_each(haywardnag_output, &haywardnag->outputs, link) {
		if (haywardnag_output->wl_output == output) {
			hayward_log(HAYWARD_DEBUG, "Surface enter on output %s",
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	if (strcmp(interface, wl_compositor_interface.name) == 0) {
		haywardnag->compositor = wl_registry_bind(registry, name,
		...
		haywardnag->layer_shell = wl_registry_bind(
...
void haywardnag_setup(struct haywardnag *haywardnag) {
	...
	if (!haywardnag->output && haywardnag->type->output) {
		hayward_log(HAYWARD_ERROR, "Output '%s' not found", haywardnag->type->output);
		haywardnag_destroy(haywardnag);
		exit(EXIT_FAILURE);
	}
	...
	haywardnag->layer_surface = zwlr_layer_shell_v1_get_layer_surface(
			...
			haywardnag->type->layer,
			"haywardnag");
	assert(haywardnag->layer_surface);
	zwlr_layer_surface_v1_add_listener(haywardnag->layer_surface,
			&layer_surface_listener, haywardnag);
	zwlr_layer_surface_v1_set_anchor(haywardnag->layer_surface,
			haywardnag->type->anchors);
