...
static bool i3bar_parse_json_color(json_object *json, uint32_t *color) {
	...
	if (!color_set) {
		wmiiv_log(WMIIV_ERROR, "Ignoring invalid block hexadecimal color string: %s", hexstring);
	}
	return color_set;
}
...
bool i3bar_handle_readable(struct status_line *status) {
	while (!status->started) { // look for opening bracket
		for (size_t c = 0; c < status->buffer_index; ++c) {
			if (status->buffer[c] == '[') {
				...
				wmiiv_log(WMIIV_DEBUG, "Invalid i3bar json: expected '[' but encountered '%c'",
						status->buffer[c]);
		...
		if (status->expecting_comma) {
			...
			if (err == json_tokener_success) {
				...
				*last_char_pos = '\0';
				...
				wmiiv_log(WMIIV_DEBUG, "Received i3bar json: '%s%c'",
				...
				*last_char_pos = last_char;
				...
				wmiiv_log(WMIIV_DEBUG, "Failed to parse i3bar json - %s: '%s%c'",
	...
	if (last_object) {
		wmiiv_log(WMIIV_DEBUG, "Rendering last received json");
...
enum hotspot_event_handling i3bar_block_send_click(struct status_line *status,
	...
	wmiiv_log(WMIIV_DEBUG, "block %s clicked", block->name);
