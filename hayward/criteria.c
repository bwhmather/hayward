...
static bool pattern_create(struct pattern **pattern, char *value) {
	*pattern = calloc(1, sizeof(struct pattern));
	if (!*pattern) {
		hayward_log(HAYWARD_ERROR, "Failed to allocate pattern");
	}
...
#if HAVE_XWAYLAND
static bool view_has_window_type(struct hayward_view *view, enum atom_name name) {
...
static int cmp_urgent(const void *_a, const void *_b) {
	struct hayward_view *a = *(void **)_a;
	struct hayward_view *b = *(void **)_b;
...
static void find_urgent_iterator(struct hayward_window *container, void *data) {
...
static bool criteria_matches_container(struct criteria *criteria,
	...
	if (criteria->container_mark) {
		bool exists = false;
		struct hayward_window *container = container;
...
static bool criteria_matches_view(struct criteria *criteria,
	...
	struct hayward_view *focused = focus ? focus->view : NULL;
...
list_t *criteria_for_view(struct hayward_view *view, enum criteria_type types) {
...
static void criteria_get_windows_iterator(struct hayward_window *container,
		void *data) {
...
static bool parse_token(struct criteria *criteria, char *name, char *value) {
	...
	case T_CON_ID:
		if (strcmp(value, "__focused__") == 0) {
			struct hayward_seat *seat = input_manager_current_seat();
			struct hayward_window *focus = seat_get_focused_container(seat);
			struct hayward_view *view = focus ? focus->view : NULL;
	...
	*error_arg = NULL;
	...
	while (*head && *head != ']') {
		...
		if (*head == '=') {
			...
			unescape(value);
			hayward_log(HAYWARD_DEBUG, "Found pair: %s=%s", name, value);
		}
