...
static void log_status(enum libinput_config_status status) {
	if (status != LIBINPUT_CONFIG_STATUS_SUCCESS) {
		hayward_log(HAYWARD_ERROR, "Failed to apply libinput config: %s",
...
static bool set_calibration_matrix(struct libinput_device *dev, float mat[6]) {
	...
	if (changed) {
		hayward_log(HAYWARD_DEBUG, "calibration_set_matrix(%f, %f, %f, %f, %f, %f)",
...
void hayward_input_configure_libinput_device(struct hayward_input_device *input_device) {
	...
	hayward_log(HAYWARD_DEBUG, "hayward_input_configure_libinput_device('%s' on '%s')",
...
void hayward_input_reset_libinput_device(struct hayward_input_device *input_device) {
	...
	struct libinput_device *device =
		wlr_libinput_get_device_handle(input_device->wlr_device);
	hayward_log(HAYWARD_DEBUG, "hayward_input_reset_libinput_device(%s)",
...
bool hayward_libinput_device_is_builtin(struct hayward_input_device *hayward_device) {
