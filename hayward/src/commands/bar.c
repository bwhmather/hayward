...
static const struct cmd_handler bar_config_handlers[] = {
    {"id", bar_cmd_id},
    {"haywardbar_command", bar_cmd_haywardbar_command},
};
...
cmd_bar(int argc, char **argv) {
    ...
    } else if (!config->reading && strcmp(argv[0], "mode") != 0 && strcmp(argv[0], "hidden_state") != 0) {
    ...
    if (!config->reading && config->current_bar) {
        ...
        if (id) {
            load_haywardbar(config->current_bar);
        }
        config->current_bar = NULL;
    }
