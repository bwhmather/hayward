...
bar_cmd_binding_mode_indicator(int argc, char **argv) {
    ...
    if (config->current_bar->binding_mode_indicator) {
        hwd_log(HWD_DEBUG, "Enabling binding mode indicator on bar: %s", config->current_bar->id);
    } else {
        hwd_log(HWD_DEBUG, "Disabling binding mode indicator on bar: %s", config->current_bar->id);
    }
