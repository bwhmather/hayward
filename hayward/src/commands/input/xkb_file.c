...
struct cmd_results *input_cmd_xkb_file(int argc, char **argv) {
	...
	if (strcmp(argv[0], "-") == 0) {
		...
		if (!can_access) {
			hayward_log_errno(HAYWARD_ERROR, "Unable to access xkb file '%s'",
					ic->xkb_file);
			config_add_haywardnag_warning("Unable to access xkb file '%s'",
					ic->xkb_file);
		}
	}
	ic->xkb_file_is_set = true;

	hayward_log(HAYWARD_DEBUG, "set-xkb_file for config: %s file: %s",
