...
void ipc_init(struct hayward_server *server) {
	...
	// We want to use socket name set by user, not existing socket from another hayward instance.
...
int ipc_handle_connection(int fd, uint32_t mask, void *data) {
	...
	if (client_fd == -1) {
		hayward_log_errno(HAYWARD_ERROR, "Unable to accept IPC client connection");
		return 0;
	}
	...
	if ((flags = fcntl(client_fd, F_GETFD)) == -1
		...
		hayward_log_errno(HAYWARD_ERROR, "Unable to set CLOEXEC on IPC client socket");
		close(client_fd);
		return 0;
	}
	if ((flags = fcntl(client_fd, F_GETFL)) == -1
			|| fcntl(client_fd, F_SETFL, flags|O_NONBLOCK) == -1) {
		hayward_log_errno(HAYWARD_ERROR, "Unable to set NONBLOCK on IPC client socket");
		close(client_fd);
		return 0;
	}
	...
	if (!client->write_buffer) {
		hayward_log(HAYWARD_ERROR, "Unable to allocate ipc client write buffer");
		close(client_fd);
		return 0;
	}
...
int ipc_client_handle_readable(int client_fd, uint32_t mask, void *data) {
	...
	if (ioctl(client_fd, FIONREAD, &read_available) == -1) {
		hayward_log_errno(HAYWARD_INFO, "Unable to read IPC socket buffer size");
		ipc_client_disconnect(client);
		return 0;
	}
	...
	if (received == -1) {
		hayward_log_errno(HAYWARD_INFO, "Unable to receive header from IPC client");
		ipc_client_disconnect(client);
		return 0;
	}
...
static void ipc_send_event(const char *json_string, ipc_command_type event) {
	...
	for (int i = 0; i < ipc_client_list->length; i++) {
		...
		if (!ipc_send_reply(client, event, json_string,
			...
			hayward_log_errno(HAYWARD_INFO, "Unable to send reply to IPC client");
...
int ipc_client_handle_writable(int client_fd, uint32_t mask, void *data) {
	...
	if (written == -1 && errno == EAGAIN) {
		...
		hayward_log_errno(HAYWARD_INFO, "Unable to send data from queue to IPC client");
		ipc_client_disconnect(client);
		return 0;
	}
...
static void ipc_get_workspaces_callback(struct hayward_workspace *workspace,
		void *data) {
...
void ipc_client_handle_command(struct ipc_client *client, uint32_t payload_length,
	...
	if (payload_length > 0) {
		...
		if (received == -1)
		{
			hayward_log_errno(HAYWARD_INFO, "Unable to receive payload from IPC client");
			ipc_client_disconnect(client);
			free(buf);
			return;
		}
	...
	case IPC_SUBSCRIBE:
	{
		...
		for (size_t i = 0; i < json_object_array_length(request); i++) {
			...
			if (strcmp(event_type, "workspace") == 0) {
				...
				hayward_log(HAYWARD_INFO, "Unsupported event type in subscribe request");
				goto exit_cleanup;
			}
		}
	...
	case IPC_SYNC:
	{
		// It was decided hayward will not support this, just return success:false
		const char msg[] = "{\"success\": false}";
		ipc_send_reply(client, payload_type, msg, strlen(msg));
		goto exit_cleanup;
	}
...
bool ipc_send_reply(struct ipc_client *client, ipc_command_type payload_type,
	...
	if (client->write_buffer_size > 4e6) { // 4 MB
		hayward_log(HAYWARD_ERROR, "Client write buffer too big (%zu), disconnecting client",
				client->write_buffer_size);
		ipc_client_disconnect(client);
		return false;
	}
	...
	if (!new_buffer) {
		hayward_log(HAYWARD_ERROR, "Unable to reallocate ipc client write buffer");
		ipc_client_disconnect(client);
		return false;
	}
	...
	hayward_log(HAYWARD_DEBUG, "Added IPC reply of type 0x%x to client %d queue: %s",
