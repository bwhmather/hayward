...
bool
loop_remove_fd(struct loop *loop, int fd) {
    for (int i = 0; i < loop->fd_length; ++i) {
        if (loop->fds[i].fd == fd) {
...
bool
loop_remove_timer(struct loop *loop, struct loop_timer *timer) {
    for (int i = 0; i < loop->timers->length; ++i) {
