...
arrange_layers(struct hayward_output *output) {
    ...
    arrange_surface(
        output, &full_area, &usable_area, output->layers.shell_background
    );
    arrange_surface(
        output, &full_area, &usable_area, output->layers.shell_bottom
    );
    arrange_surface(output, &full_area, &usable_area, output->layers.shell_top);
    arrange_surface(
        output, &full_area, &usable_area, output->layers.shell_overlay
    );
...
hayward_layer_get_scene(
    struct hayward_output *output, enum zwlr_layer_shell_v1_layer type
) {
    switch (type) {
...
hayward_layer_surface_create(struct wlr_scene_layer_surface_v1 *scene) {
    struct hayward_layer_surface *layer_surface =
        calloc(1, sizeof(struct hayward_layer_surface));
    hayward_assert(layer_surface != NULL, "Allocation failed");

    struct wlr_scene_tree *popups = wlr_scene_tree_create(root->layers.popups);
    hayward_assert(popups != NULL, "Allocation failed");
...
find_mapped_layer_by_client(
    struct wl_client *client, struct hayward_output *ignore_output
) {
...
handle_node_destroy(struct wl_listener *listener, void *data) {
    ...
    struct wl_client *client =
        wl_resource_get_client(layer_surface->layer_surface->resource);
    bool set_focus = seat->exclusive_client == client;
    if (set_focus) {
        struct hayward_layer_surface *consider_layer =
...
handle_surface_commit(struct wl_listener *listener, void *data) {
    ...
    if (committed & WLR_LAYER_SURFACE_V1_STATE_LAYER) {
        ...
        struct wlr_scene_tree *output_layer =
            hayward_layer_get_scene(layer_surface->output, layer_type);
        wlr_scene_node_reparent(
            &layer_surface->scene->tree->node, output_layer
        );
    }
...
popup_unconstrain(struct hayward_layer_popup *popup) {
    ...
    struct hayward_output *output = popup->toplevel->output;
...
create_popup(
    struct wlr_xdg_popup *wlr_popup, struct hayward_layer_surface *toplevel,
...
handle_new_surface(struct wl_listener *listener, void *data) {
    ...
    if (!wlr_layer_surface->output) {
        ...
        struct hayward_output *output = root->outputs->items[0];
        wlr_layer_surface->output = output->wlr_output;
    }

    struct hayward_output *output = wlr_layer_surface->output->data;

    enum zwlr_layer_shell_v1_layer layer_type =
        wlr_layer_surface->pending.layer;
    struct wlr_scene_tree *output_layer =
        hayward_layer_get_scene(output, layer_type);
    struct wlr_scene_layer_surface_v1 *scene_surface =
        wlr_scene_layer_surface_v1_create(output_layer, wlr_layer_surface);
    if (!scene_surface) {
        hayward_log(HAYWARD_ERROR, "Could not allocate a wlr_layer_surface_v1");
        return;
    }

    struct hayward_layer_surface *surface =
        hayward_layer_surface_create(scene_surface);
    ...
    wl_signal_add(
        &scene_surface->tree->node.events.destroy, &surface->node_destroy
    );
