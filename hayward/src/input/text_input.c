...
static struct hayward_text_input *relay_get_focusable_text_input(
		struct hayward_input_method_relay *relay) {
	struct hayward_text_input *text_input = NULL;
...
static struct hayward_text_input *relay_get_focused_text_input(
		struct hayward_input_method_relay *relay) {
	struct hayward_text_input *text_input = NULL;
	wl_list_for_each(text_input, &relay->text_inputs, link) {
...
static void handle_im_commit(struct wl_listener *listener, void *data) {
	struct hayward_input_method_relay *relay = wl_container_of(listener, relay,
		input_method_commit);
...
static void relay_send_im_state(struct hayward_input_method_relay *relay,
		struct wlr_text_input_v3 *input) {
	struct wlr_input_method_v2 *input_method = relay->input_method;
	if (!input_method) {
		hayward_log(HAYWARD_INFO, "Sending IM_DONE but im is gone");
		return;
	}
...
static void handle_text_input_enable(struct wl_listener *listener, void *data) {
	struct hayward_text_input *text_input = wl_container_of(listener, text_input,
		text_input_enable);
	if (text_input->relay->input_method == NULL) {
		hayward_log(HAYWARD_INFO, "Enabling text input when input method is gone");
		return;
	}
...
static void handle_text_input_commit(struct wl_listener *listener,
	...
	if (!text_input->input->current_enabled) {
		hayward_log(HAYWARD_INFO, "Inactive text input tried to commit an update");
		return;
	}
	hayward_log(HAYWARD_DEBUG, "Text input committed update");
	if (text_input->relay->input_method == NULL) {
		hayward_log(HAYWARD_INFO, "Text input committed, but input method is gone");
		return;
	}
	relay_send_im_state(text_input->relay, text_input->input);
}

static void relay_disable_text_input(struct hayward_input_method_relay *relay,
		struct hayward_text_input *text_input) {
	if (relay->input_method == NULL) {
		hayward_log(HAYWARD_DEBUG, "Disabling text input, but input method is gone");
		return;
	}
...
static void handle_text_input_disable(struct wl_listener *listener,
	...
	if (text_input->input->focused_surface == NULL) {
		hayward_log(HAYWARD_DEBUG, "Disabling text input, but no longer focused");
		return;
	}
...
struct hayward_text_input *hayward_text_input_create(
	...
	struct hayward_text_input *input = calloc(1, sizeof(struct hayward_text_input));
	if (!input) {
		return NULL;
	}
...
static void relay_handle_input_method(struct wl_listener *listener,
	...
	if (relay->input_method != NULL) {
		hayward_log(HAYWARD_INFO, "Attempted to connect second input method to a seat");
	...
	struct hayward_text_input *text_input = relay_get_focusable_text_input(relay);
	if (text_input) {
	...
	wl_list_for_each(text_input, &relay->text_inputs, link) {
		if (text_input->pending_focused_surface) {
			...
			if (surface != text_input->input->focused_surface) {
				...
				hayward_log(HAYWARD_DEBUG, "IM relay set_focus already focused");
				continue;
			}
		}
