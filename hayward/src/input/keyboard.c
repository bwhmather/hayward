 ...
 */
static bool state_erase_key(struct hayward_shortcut_state *state,
		uint32_t keycode) {
	bool found = false;
	size_t j = 0;
 ...
 */
static void state_add_key(struct hayward_shortcut_state *state,
 ...
 */
static bool update_shortcut_state(struct hayward_shortcut_state *state,
 ...
 */
static void get_active_binding(const struct hayward_shortcut_state *state,
		list_t *bindings, struct hayward_binding **current_binding,
 ...
 */
static bool keyboard_execute_compositor_binding(struct hayward_keyboard *keyboard,
 ...
 */
static size_t keyboard_keysyms_translated(struct hayward_keyboard *keyboard,
 ...
 */
static size_t keyboard_keysyms_raw(struct hayward_keyboard *keyboard,
		xkb_keycode_t keycode, const xkb_keysym_t **keysyms,
		uint32_t *modifiers) {
	struct wlr_input_device *device =
		keyboard->seat_device->input_device->wlr_device;
...
static void update_keyboard_state(struct hayward_keyboard *keyboard,
...
/**
 * Get keyboard grab of the seat from hayward_keyboard if we should forward events
 * to it.
 *
...
static struct wlr_input_method_keyboard_grab_v2 *keyboard_get_im_grab(
	...
	struct wlr_input_method_v2 *input_method = keyboard->seat_device->
		hayward_seat->im_relay.input_method;
...
static void handle_key_event(struct hayward_keyboard *keyboard,
	...
	bool shortcuts_inhibited = hayward_inhibitor && hayward_inhibitor->inhibitor->active;
...
static int handle_keyboard_repeat(void *data) {
	...
	if (keyboard->repeat_binding) {
		...
		seat_execute_command(keyboard->seat_device->hayward_seat,
...
static void hayward_keyboard_group_remove(struct hayward_keyboard *keyboard) {
	...
	hayward_log(HAYWARD_DEBUG, "Removing keyboard %s from group %p",
	...
	if (wl_list_empty(&wlr_group->devices)) {
		hayward_log(HAYWARD_DEBUG, "Destroying empty keyboard group %p", (void *) wlr_group);
...
static void hayward_keyboard_group_remove_invalid(struct hayward_keyboard *keyboard) {
	struct hayward_input_device *device = keyboard->seat_device->input_device;
	struct wlr_keyboard *wlr_keyboard = device->wlr_device->keyboard;
...
static void hayward_keyboard_group_add(struct hayward_keyboard *keyboard) {
	...
	wl_list_for_each(group, &seat->keyboard_groups, link) {
		...
		case KEYBOARD_GROUP_SMART:;
			...
			if (wlr_keyboard_keymaps_match(keyboard->keymap,
				...
				hayward_log(HAYWARD_DEBUG, "Adding keyboard %s to group %p",
						device->identifier, (void *) wlr_group);
	...
	wlr_keyboard_set_keymap(&hayward_group->wlr_group->keyboard, keyboard->keymap);
	wlr_keyboard_set_repeat_info(&hayward_group->wlr_group->keyboard,
			keyboard->repeat_rate, keyboard->repeat_delay);
	hayward_log(HAYWARD_DEBUG, "Created keyboard group %p", (void *) hayward_group->wlr_group);

	hayward_group->seat_device = calloc(1, sizeof(struct hayward_seat_device));
	if (!hayward_group->seat_device) {
		hayward_log(HAYWARD_ERROR, "Failed to allocate hayward_seat_device for group");
		goto cleanup;
	}
	...
	if (!hayward_group->seat_device->input_device) {
		hayward_log(HAYWARD_ERROR, "Failed to allocate hayward_input_device for group");
		goto cleanup;
	}
	...
	if (!hayward_keyboard_create(seat, hayward_group->seat_device)) {
		hayward_log(HAYWARD_ERROR, "Failed to allocate hayward_keyboard for group");
		goto cleanup;
	}

	hayward_log(HAYWARD_DEBUG, "Adding keyboard %s to group %p",
			device->identifier, (void *) hayward_group->wlr_group);
	...
	wl_signal_add(&hayward_group->wlr_group->keyboard.events.key,
...
void hayward_keyboard_configure(struct hayward_keyboard *keyboard) {
	...
	if (!keymap) {
		hayward_log(HAYWARD_ERROR, "Failed to compile keymap. Attempting defaults");
