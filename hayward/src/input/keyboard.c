...
handle_xkb_context_log(
    ...
    if (!error) {
        hayward_log(
            HAYWARD_ERROR, "Failed to allocate libxkbcommon log message"
        );
        return;
    }
...
hayward_keyboard_compile_keymap(struct input_config *ic, char **error) {
    ...
    if (ic && ic->xkb_file) {
        ...
        if (!keymap_file) {
            hayward_log_errno(
                HAYWARD_ERROR, "cannot read xkb file %s", ic->xkb_file
            );
            if (error) {
                size_t len = snprintf(
                                 NULL, 0, "cannot read xkb file %s: %s",
                ...
                *error = malloc(len);
                if (*error) {
                    snprintf(
                        *error, len, "cannot read xkb_file %s: %s",
                        ic->xkb_file, strerror(errno)
                    );
                }
            }
        ...
        keymap = xkb_keymap_new_from_file(
            context, keymap_file, XKB_KEYMAP_FORMAT_TEXT_V1,
            XKB_KEYMAP_COMPILE_NO_FLAGS
        );

        if (fclose(keymap_file) != 0) {
            hayward_log_errno(
                HAYWARD_ERROR, "Failed to close xkb file %s", ic->xkb_file
            );
        }
    } else {
 ...
 */
static bool
state_erase_key(struct hayward_shortcut_state *state, uint32_t keycode) {
    bool found = false;
    size_t j = 0;
    ...
    for (int i = 0; i < bindings->length; ++i) {
        ...
        if (*current_binding) {
            ...
            if (current_input == binding_input &&
                ...
                hayward_log(
                    HAYWARD_DEBUG, "Encountered conflicting bindings %d and %d",
...
keyboard_keysyms_translated(
    ...
    *modifiers = *modifiers & ~consumed;
    ...
    return xkb_state_key_get_syms(
        device->keyboard->xkb_state, keycode, keysyms
    );
}
...
keyboard_keysyms_raw(
    ...
    *modifiers = wlr_keyboard_get_modifiers(device->keyboard);
    ...
    return xkb_keymap_key_get_syms_by_level(
        device->keyboard->keymap, keycode, layout_index, 0, keysyms
    );
}
...
update_keyboard_state(
    ...
    keyinfo->raw_keysyms_len = keyboard_keysyms_raw(
        keyboard, keyinfo->keycode, &keyinfo->raw_keysyms,
        &keyinfo->raw_modifiers
    );

    keyinfo->translated_keysyms_len = keyboard_keysyms_translated(
        keyboard, keyinfo->keycode, &keyinfo->translated_keysyms,
    ...
    update_shortcut_state(
        &keyboard->state_keycodes, raw_keycode, keystate, keyinfo->keycode,
        keyinfo->code_modifiers
    );
    for (size_t i = 0; i < keyinfo->raw_keysyms_len; ++i) {
        update_shortcut_state(
            &keyboard->state_keysyms_raw, raw_keycode, keystate,
            keyinfo->raw_keysyms[i], keyinfo->code_modifiers
        );
    }
...
/**
 * Get keyboard grab of the seat from hayward_keyboard if we should forward
 * events to it.
 *
...
keyboard_get_im_grab(struct hayward_keyboard *keyboard) {
    struct wlr_input_method_v2 *input_method =
        keyboard->seat_device->hayward_seat->im_relay.input_method;
...
handle_key_event(
    ...
    bool shortcuts_inhibited =
        hayward_inhibitor && hayward_inhibitor->inhibitor->active;
    ...
    struct hayward_binding *binding_released = NULL;
    get_active_binding(
        &keyboard->state_keycodes, config->current_mode->keycode_bindings,
        &binding_released, keyinfo.code_modifiers, true, input_inhibited,
        shortcuts_inhibited, device_identifier, exact_identifier,
        keyboard->effective_layout
    );
    get_active_binding(
        &keyboard->state_keysyms_raw, config->current_mode->keysym_bindings,
        &binding_released, keyinfo.raw_modifiers, true, input_inhibited,
        shortcuts_inhibited, device_identifier, exact_identifier,
        keyboard->effective_layout
    );
    get_active_binding(
    ...
    if (event->state == WL_KEYBOARD_KEY_STATE_PRESSED) {
        get_active_binding(
            &keyboard->state_keycodes, config->current_mode->keycode_bindings,
            &binding, keyinfo.code_modifiers, false, input_inhibited,
            shortcuts_inhibited, device_identifier, exact_identifier,
            keyboard->effective_layout
        );
        get_active_binding(
            &keyboard->state_keysyms_raw, config->current_mode->keysym_bindings,
            &binding, keyinfo.raw_modifiers, false, input_inhibited,
            shortcuts_inhibited, device_identifier, exact_identifier,
            keyboard->effective_layout
        );
        get_active_binding(
    ...
    if (binding && !(binding->flags & BINDING_NOREPEAT) &&
        ...
        if (wl_event_source_timer_update(
                ...
                wlr_device->keyboard->repeat_info.delay
            ) < 0) {
            hayward_log(HAYWARD_DEBUG, "failed to set key repeat timer");
        }
...
handle_keyboard_group_leave(struct wl_listener *listener, void *data) {
    ...
    wl_array_for_each(keycode, keycodes) {
        struct key_info keyinfo;
        update_keyboard_state(
            keyboard, *keycode, WL_KEYBOARD_KEY_STATE_RELEASED, &keyinfo
        );
...
handle_keyboard_repeat(void *data) {
    ...
    if (keyboard->repeat_binding) {
        if (wlr_device->repeat_info.rate > 0) {
            ...
            if (wl_event_source_timer_update(
                    keyboard->key_repeat_source,
                    1000 / wlr_device->repeat_info.rate
                ) < 0) {
                hayward_log(HAYWARD_DEBUG, "failed to update key repeat timer");
            }
        }

        seat_execute_command(
            keyboard->seat_device->hayward_seat, keyboard->repeat_binding
        );
    }
    return 0;
}
    ...
    if (!wlr_device->keyboard->group) {
        ...
        if (kb_grab) {
            ...
            wlr_input_method_keyboard_grab_v2_send_modifiers(
                kb_grab, &wlr_device->keyboard->modifiers
            );
        } else {
            struct wlr_seat *wlr_seat =
                keyboard->seat_device->hayward_seat->wlr_seat;
            wlr_seat_set_keyboard(wlr_seat, wlr_device->keyboard);
            wlr_seat_keyboard_notify_modifiers(
                wlr_seat, &wlr_device->keyboard->modifiers
            );
        }
...
hayward_keyboard_create(
    ...
    keyboard->key_repeat_source = wl_event_loop_add_timer(
        server.wl_event_loop, handle_keyboard_repeat, keyboard
    );
...
static bool
repeat_info_match(struct hayward_keyboard *a, struct wlr_keyboard *b) {
...
hayward_keyboard_group_remove(struct hayward_keyboard *keyboard) {
    ...
    hayward_log(
        HAYWARD_DEBUG, "Removing keyboard %s from group %p", device->identifier,
        (void *)wlr_group
    );
    ...
    if (wl_list_empty(&wlr_group->devices)) {
        hayward_log(
            HAYWARD_DEBUG, "Destroying empty keyboard group %p",
            (void *)wlr_group
        );
        struct hayward_keyboard_group *hayward_group = wlr_group->data;
        wlr_group->data = NULL;
        wl_list_remove(&hayward_group->link);
        wl_list_remove(&hayward_group->keyboard_key.link);
        wl_list_remove(&hayward_group->keyboard_modifiers.link);
        wl_list_remove(&hayward_group->enter.link);
        wl_list_remove(&hayward_group->leave.link);
        hayward_keyboard_destroy(hayward_group->seat_device->keyboard);
        free(hayward_group->seat_device->input_device);
        free(hayward_group->seat_device);
        free(hayward_group);
...
static void
hayward_keyboard_group_remove_invalid(struct hayward_keyboard *keyboard) {
    struct hayward_input_device *device = keyboard->seat_device->input_device;
    struct wlr_keyboard *wlr_keyboard = device->wlr_device->keyboard;
...
hayward_keyboard_group_add(struct hayward_keyboard *keyboard) {
    ...
    wl_list_for_each(group, &seat->keyboard_groups, link) {
        ...
        case KEYBOARD_GROUP_SMART:;
            ...
            if (wlr_keyboard_keymaps_match(
                    keyboard->keymap, wlr_group->keyboard.keymap
                ) &&
                repeat_info_match(keyboard, &wlr_group->keyboard)) {
                hayward_log(
                    HAYWARD_DEBUG, "Adding keyboard %s to group %p",
    ...
    struct hayward_keyboard_group *hayward_group =
        calloc(1, sizeof(struct hayward_keyboard_group));
    if (!hayward_group) {
        hayward_log(HAYWARD_ERROR, "Failed to allocate hayward_keyboard_group");
        return;
    }

    hayward_group->wlr_group = wlr_keyboard_group_create();
    if (!hayward_group->wlr_group) {
        hayward_log(HAYWARD_ERROR, "Failed to create keyboard group");
        goto cleanup;
    }
    hayward_group->wlr_group->data = hayward_group;
    wlr_keyboard_set_keymap(
        &hayward_group->wlr_group->keyboard, keyboard->keymap
    );
    wlr_keyboard_set_repeat_info(
        &hayward_group->wlr_group->keyboard, keyboard->repeat_rate,
        keyboard->repeat_delay
    );
    hayward_log(
        HAYWARD_DEBUG, "Created keyboard group %p",
        (void *)hayward_group->wlr_group
    );

    hayward_group->seat_device = calloc(1, sizeof(struct hayward_seat_device));
    if (!hayward_group->seat_device) {
        hayward_log(
            HAYWARD_ERROR, "Failed to allocate hayward_seat_device for group"
        );
        goto cleanup;
    }
    hayward_group->seat_device->hayward_seat = seat;

    hayward_group->seat_device->input_device =
        calloc(1, sizeof(struct hayward_input_device));
    if (!hayward_group->seat_device->input_device) {
        hayward_log(
            HAYWARD_ERROR, "Failed to allocate hayward_input_device for group"
        );
        goto cleanup;
    }
    hayward_group->seat_device->input_device->wlr_device =
        &hayward_group->wlr_group->keyboard.base;

    if (!hayward_keyboard_create(seat, hayward_group->seat_device)) {
        hayward_log(
            HAYWARD_ERROR, "Failed to allocate hayward_keyboard for group"
        );
        goto cleanup;
    }

    hayward_log(
        HAYWARD_DEBUG, "Adding keyboard %s to group %p", device->identifier,
        (void *)hayward_group->wlr_group
    );
    wlr_keyboard_group_add_keyboard(hayward_group->wlr_group, wlr_keyboard);

    wl_list_insert(&seat->keyboard_groups, &hayward_group->link);

    wl_signal_add(
        &hayward_group->wlr_group->keyboard.events.key,
        &hayward_group->keyboard_key
    );
    hayward_group->keyboard_key.notify = handle_keyboard_group_key;

    wl_signal_add(
        &hayward_group->wlr_group->keyboard.events.modifiers,
        &hayward_group->keyboard_modifiers
    );
    hayward_group->keyboard_modifiers.notify = handle_keyboard_group_modifiers;

    wl_signal_add(
        &hayward_group->wlr_group->events.enter, &hayward_group->enter
    );
    hayward_group->enter.notify = handle_keyboard_group_enter;

    wl_signal_add(
        &hayward_group->wlr_group->events.leave, &hayward_group->leave
    );
    hayward_group->leave.notify = handle_keyboard_group_leave;
    return;

cleanup:
    if (hayward_group && hayward_group->wlr_group) {
        wlr_keyboard_group_destroy(hayward_group->wlr_group);
    }
    free(hayward_group->seat_device->keyboard);
    free(hayward_group->seat_device->input_device);
    free(hayward_group->seat_device);
    free(hayward_group);
}
...
hayward_keyboard_configure(struct hayward_keyboard *keyboard) {
    ...
    struct xkb_keymap *keymap =
        hayward_keyboard_compile_keymap(input_config, NULL);
    if (!keymap) {
        hayward_log(
            HAYWARD_ERROR, "Failed to compile keymap. Attempting defaults"
        );
        keymap = hayward_keyboard_compile_keymap(NULL, NULL);
        if (!keymap) {
            hayward_log(
                HAYWARD_ERROR,
    ...
    if (keymap_changed || repeat_info_changed || config->reloading) {
        ...
        hayward_keyboard_group_remove_invalid(keyboard);

        wlr_keyboard_set_keymap(wlr_device->keyboard, keyboard->keymap);
        wlr_keyboard_set_repeat_info(
            wlr_device->keyboard, keyboard->repeat_rate, keyboard->repeat_delay
        );

        if (!wlr_device->keyboard->group) {
            hayward_keyboard_group_add(keyboard);
        }
        ...
        if (locked_mods) {
            wlr_keyboard_notify_modifiers(
                wlr_device->keyboard, 0, 0, locked_mods, 0
            );
            uint32_t leds = 0;
    ...
    struct wlr_seat *seat = keyboard->seat_device->hayward_seat->wlr_seat;
    ...
    wl_signal_add(
        &wlr_device->keyboard->events.modifiers, &keyboard->keyboard_modifiers
    );
...
hayward_keyboard_destroy(struct hayward_keyboard *keyboard) {
    ...
    struct wlr_seat *wlr_seat = keyboard->seat_device->hayward_seat->wlr_seat;
    struct hayward_input_device *device = keyboard->seat_device->input_device;
