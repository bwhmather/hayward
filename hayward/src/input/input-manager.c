...
static bool
device_is_touchpad(struct hayward_input_device *device) {
...
static void
apply_input_type_config(struct hayward_input_device *input_device) {
...
static void
handle_inhibit_activate(struct wl_listener *listener, void *data) {
...
handle_keyboard_shortcuts_inhibitor_destroy(
    ...
    // hayward_seat::keyboard_shortcuts_inhibitors
    wl_list_remove(&hayward_inhibitor->link);
...
input_manager_create(
    ...
    input->virtual_keyboard =
        wlr_virtual_keyboard_manager_v1_create(wl_display);
    wl_signal_add(
    ...
    input->inhibit = wlr_input_inhibit_manager_create(wl_display);
    ...
    input->keyboard_shortcuts_inhibit =
        wlr_keyboard_shortcuts_inhibit_v1_create(wl_display);
...
input_manager_reset_input(struct hayward_input_device *input_device) {
    hayward_input_reset_libinput_device(input_device);
    struct hayward_seat *seat = NULL;
    wl_list_for_each(seat, &server.input->seats, link) {
...
input_manager_reset_all_inputs(void) {
    ...
    wl_list_for_each(seat, &server.input->seats, link) {
        ...
        wl_list_for_each(group, &seat->keyboard_groups, link) {
            hayward_keyboard_disarm_key_repeat(group->seat_device->keyboard);
        }
    }
}
